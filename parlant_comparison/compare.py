...
class ComparisonRunner:
    ...
    async def initialize_parlant_server(self):
        ...
        try:
            ...
            # Matches SDK test structure: create agents, EXIT context, then HTTP serves
            async def start_server_with_agents():
                try:
                    async with server:
                        ...
                        print(f"✓ Created {len(self.parlant_agents)} agent(s)")

                    # Context exits here - HTTP server starts now!
                    # This is the key: serve_app runs after exiting the context
                    print("✓ Agents created, HTTP server starting...")
